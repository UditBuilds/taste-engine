...
Follow-up to this audit, executed across two briefs:
`briefs/regenerate_stale.md` (stopped at its own B3 verdict gate) and
`briefs/finish_regeneration.md` (resumed from that stop and finished it).
Appended only — nothing above this line was changed.

### What was regenerated/updated, and where it landed

| file | action | commit |
|---|---|---|
| `src/taste_engine/evaluate.py` | remaining `exclude_top=50`/`k=20` literals read from config (`regenerate_stale.md` Part A) | `efcd6eb` |
| `reports/dormancy_signals.md` | regenerated at `MAX(watched_at)` | `c51bda2` |
| `scripts/dormancy_probe.py` | fixed hardcoded N=365 prose (`8`/`0.27%`/`at most 1`), added a verdict paragraph | `c51bda2` |
| `tests/test_dormancy_probe.py` | new regression test, `TestN365ProseMatchesTable` | `c51bda2` |
| `reports/dormancy_probe.md` | regenerated at `MAX(watched_at)` with the fixed script | `c51bda2` |
| `scripts/recency_exclusion.py` | new — committed, `--as-of` optional, defaults to `MAX(watched_at)` | `925f091` |
| `reports/recency_exclusion.md` | tables and prose updated from the `MAX(watched_at)` run | `925f091` |
| `reports/listening_test.md` | number-only citation edit | `925f091` |

### Every moved figure, old → new

| figure | old | new |
|---|---|---|
| `dormancy_signals.md` §4 headline | 177 of 180 (98.3%) | **197 of 198 (99.5%)** |
| `dormancy_probe.md` N=365, library-wide eligible | 8 of 2,918 (0.27%) | **0 of 2,918 (0.0%)** |
| `dormancy_probe.md` N=365, in a qualifying cluster | at most 1 | **0** |
| `recency_exclusion.md` qualifying clusters | 8 | **10** |
| `recency_exclusion.md` total candidate pool (§3 aggregate) | 139 | **198** |
| `recency_exclusion.md` survivors @30d (aggregate) | 2 | **1** |
| `recency_exclusion.md` §5 reconfirmation | 137 of 139 (98.6%) | **197 of 198 (99.5%)** |
| `listening_test.md:63` citation | 177 of 180 (98.3%) | **197 of 198 (99.5%)** |

Confirmed unchanged, as required (B5 of `regenerate_stale.md`, re-confirmed
in `finish_regeneration.md`'s regenerations): the 362-day/21-hour export
span and the 61.8% single-play rate — neither formula contains `as_of`.

### The pre-fix N=365 count of 8 was an artifact of the anchor bug

`reports/dormancy_probe.md`'s pre-fix `as_of` (`2026-09-16T04:42:49.99`,
its own header) sat **2.89 days** past `MAX(watched_at)`
(`2026-09-13T07:24:35`) — confirmed two independent ways during
`finish_regeneration.md`'s Part 0: direct date arithmetic, and the
"Choo Lo" track's `days_since_last_play` shift (365.8 → 362.9) between the
pre-fix report and this session's regeneration, the only N=365 track both
share. At `as_of = MAX(watched_at)`, N=365 is structurally unreachable —
no track's `days_since_last_play` can exceed the export's own ~362.9-day
span — so the 8-vs-0 move is not a small drift, it is the anchor bug's
entire effect made visible. A close-sounding but different figure, ~7.14
days, is the gap between `MAX(watched_at)` and commit `cd1b28d` (when the
bug was fixed, not the `as_of` that produced the 8-tracks report) — an
earlier draft of `finish_regeneration.md`'s own approved verdict paragraph
conflated the two; corrected to "about three days" before being inserted,
per the user's explicit choice after this was flagged.

### `dormancy_probe.py`'s hardcoded prose figures — fixed and tested

`scripts/dormancy_probe.py:451-453` hardcoded `8`, `0.27%` and `at most 1`
as literal text in the Section 5 sentence, three lines above the pool
table it describes — so the table could (and did) go to zero while the
prose kept asserting 8. Fixed to interpolate from the same N=365 pool-table
row the table itself renders. `tests/test_dormancy_probe.py`'s new
`TestN365ProseMatchesTable` parses both the rendered sentence and the
rendered table out of one real `build_report()` call and asserts they
agree, regardless of what the current figures are — a guard against this
recurring, not just a fix for today's specific numbers.

### `recency_exclusion.md` now has a committed, validated script

Previously an ad hoc script kept out of `scripts/`, by its own report's
words "never meant to be a permanent artifact." Now `scripts/recency_exclusion.py`,
validated twice before being trusted: reproduced the committed table
exactly at the original wall-clock `as_of` (8 clusters; candidates
17-13-19-14-23-17-16-20; survivors 2-0-0; §5's 137/139), *and*, separately,
at `MAX(watched_at)` its 10-cluster output matches `dormancy_signals.md`'s
independently-regenerated cluster table cell for cell.

### Rows still open

- **`reports/backfill_plan.md` (row 7) — INDETERMINATE, unchanged.** Not
  regenerated by either brief (explicitly out of scope for both: backfill
  is disabled; regenerating would publish a number for an unused feature).
  The historical anchor is still unrecoverable for the reasons this
  audit's own table already gives.
- **`reports/recency_exclusion.md` §1's "kept in a scratch location...
  never meant to be a permanent artifact"** is now false as of commit
  `925f091` (which committed `scripts/recency_exclusion.py`) and sits two
  lines above a new sentence stating the opposite. Not a number, so not
  in scope for `finish_regeneration.md`'s Part B3 to fix; flagged there,
  left as-is, not resolved here either.
- **`reports/recency_exclusion.md` §4 "Cluster-count drift"** is stale
  beyond what number substitution can fix, not merely imprecise: it
  asserts `dormancy_signals.md` recorded 10 qualifying clusters against
  "today's frame" of 8 — both are 10 now, at the shared anchor. Its
  worked example ("T-Series is cluster `12`... where it was recorded as
  cluster `11`") is also stale: `dormancy_signals.md`'s own regenerated
  table shows cluster `12` for T-Series too. The section's entire premise
  — an unexplained discrepancy between the two reports — no longer exists
  once both share an anchor. Left unreworded per instruction; a reader
  reaching §4 immediately after the corrected §3 will see an unmarked
  contradiction until a human rewrites or removes it.
- **`reports/listening_test.md:63`'s citation date, "computed
  2026-09-16"**, was deliberately left untouched alongside the corrected
  177→197/180→198/98.3%→99.5% figures. Correcting it is a judgment call
  among several candidate dates (this report's own regeneration date,
  the `MAX(watched_at)` anchor, or the commit that regenerated it) with
  no single mechanical answer, not a number substitution — left open.
- `reports/genre_coverage.md` (row 8 of the table above) remains
  ANCHOR-DEPENDENT and unregenerated — named in `regenerate_stale.md`'s
  own Part 0 as a report the audit marks anchor-dependent-and-stale but
  not on that brief's regeneration list; neither brief touched it.
